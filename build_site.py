...
def parse_front_matter_and_body(text: str) -> (Dict[str, Any], str):
    ...
    if not m:
        # no front matter, return empty fm and whole text as body
        return {}, text
...
def build_post_pages(posts: List[Dict[str, Any]]):
    ...
    for idx, post in enumerate(posts):
        ...
        # Related posts = next few posts in list
        related_items = []
        for other in posts:
            if other["slug"] == slug:
                continue
...
"""
    ...
    posts_block = "\n".join(list_html_parts) or "<p>No posts yet – the AI will generate content soon.</p>"
...
<p>Browse every article generated by the system. New guides are added automatically over time.</p>
